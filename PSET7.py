...
class NewsStory(object):
    '''
     Is an object storing 1 RSS entry.
    '''
    ...
    for trig in triggerlist:
        for story in stories:
            if trig.evaluate(story) == True:
                if not story in returned_stories:
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    """
    if triggerType == "PHRASE" or triggerType == 'TITLE' or triggerType == 'SUBJECT' or triggerType == 'SUMMARY':
        par = ' '.join(params)
        exec('trig = ' + str(triggerType).lower().capitalize() + "Trigger('" + par + "')")
        ...
        for i in params:
            par += 'triggerMap["' + i + '"], '
        exec('trig = ' + str(triggerType).lower().capitalize() + 'Trigger(' + par[:-2] + ')')
        triggerMap[name] = trig
        return trig
